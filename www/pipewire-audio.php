...
<body>
    ...
    <script>
        ...
        function RenderGroupCard(group, index) {
            ...
            html += '<input type="range" class="form-range" min="0" max="100" value="' + (group.volume || 100) + '" oninput="UpdateGroupVolumeDisplay(this)" onchange="SetGroupVolume(' + index + ', this.value)">';
        ...
        function RenderMemberRow(group, groupIndex, memberIndex) {
            ...
            html += '</select>';
            html += '</td>';
            html += '<td>' + channelMapping + '</td>';
            html += '<td>';
            html += '<div class="volume-slider-container">';
            html += '<input type="range" class="form-range" min="0" max="100" value="' + (member.volume || 100) + '" oninput="UpdateGroupVolumeDisplay(this)" onchange="SetMemberVolume(' + groupIndex + ',' + memberIndex + ', this.value)" style="min-width:80px;">';
        ...
        function SetMemberVolume(groupIndex, memberIndex, volume) {
            ...
            var member = audioGroups.groups[groupIndex].members[memberIndex];
            if (member.cardId) {
                // Find the real PipeWire sink name for this card
                var sinkName = null;
                for (var i = 0; i < availableCards.length; i++) {
                    if (availableCards[i].cardId === member.cardId) {
                        // Use FPP-style name as best guess; actual pactl targets by node name
                        sinkName = 'alsa_output.fpp_card' + availableCards[i].cardNum;
                        break;
                    }
                }
                if (sinkName) {
